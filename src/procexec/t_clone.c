...
static int child_func(void *arg) // Startup function for cloned child
{
  if (close(*((int *)arg)) == -1) {
    errExit("close");
  }
...
int main(int argc, char *argv[])
{
  ...
  // the process, but don't ignore SIGCHLD (which is ignored by defalut),
  ...
  // Parent falls through to here. Wait for child, __WCLONE is needed
